/* Storage: Upstash Redis provisioned through the Vercel Marketplace.
   NOUGHT_MEMORY=1 is only for the local test server; production never sets it. */
import { Redis } from '@upstash/redis';
let client = null;
export function redis() {
  if (client) return client;
  if (process.env.NOUGHT_MEMORY === '1') return (client = memory());
  ...
  try {
    for (const k of keys) {
      ...
      for (let i = 0; i < 60 && !ok; i++) {
        if (await R.set('nt:lock:' + k, tok, { nx: true, px: 5000 })) ok = true;
        else await new Promise(r => setTimeout(r, 80));
      }
      if (!ok) throw Error('The launchpad is busy, try again.');
      held.push(k);
    }
    return await fn();
  } finally {
    for (const k of held) { try { if ((await R.get('nt:lock:' + k)) === tok) await R.del('nt:lock:' + k); } catch {} }
  }
}
function memory() {
